...
def compute_reward(robot, obs1, obs2, params, dt_outer):
    ...
    for i in range(H):
        ...
        control_ref = unicycle_nominal_input_jit( states[i], robot.goal_torch )
        A, b = unicycle2D_qp_constraints_jit( states[i], robot.goal_torch, obs1.X_torch, obs2.X_torch, params[0], params[1], params[2] )
        ...
        # Get next state
        next_state = update_unicycle_state_jit( states[i], control, dt_outer )
                
        # Save next state and compute reward
        states.append( next_state )
        reward = reward + unicycle_compute_reward_jit( states[i+1], robot.goal_torch, control )       
...
def simulate_scenario( movie_name = 'test.mp4', adapt = True, enforce_input_constraints = False, params = [1.0, 0.8, 0.8], plot_x_lim = (-5,5), plot_y_lim = (-5,5) ):
    ...
    while (t < tf):
        ...
        if (i % outer_loop != 0): # compute control input and move the robot
            ...
            while not success:            
                reward, improve_constraints, maintain_constraints, success = compute_reward(robot, obs1, obs2, robot.params, dt_outer)
