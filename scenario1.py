...
robots.append( DoubleIntegrator3D( np.array([-3,-3,0,0,0,0]), dt, ax, nominal_plot = nominal_plot, id = 8, color='b', alpha = alpha, mode='ego', target = np.array([1.0,0,0]).reshape(-1,1), num_robots = num_robots, num_constraints = num_constraints  ) )
...
for t in range(T):    
    ...
    #                 best_controllerT3.solve( solver=cp.GUROBI )
