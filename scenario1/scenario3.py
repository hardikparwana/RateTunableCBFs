...
# robots.append( Unicycle2D( np.array([-1,-3,0, np.pi/2]), dt, ax, id = 2, nominal_plot = nominal_plot, color='b', alpha = alpha, mode='ego', target = np.array([1.2,0]).reshape(-1,1), num_robots = num_robots, num_constraints = num_constraints ) )
...
robots.append( Surveillance(np.array([4,2,height,0,0,0]), dt, ax, id = 0, cone_length = height/np.cos(cone_angle), cone_angle = cone_angle, mode = 'uncooperative', target = np.array([-1,0,0,0,0,0]).reshape(-1,1), num_robots = num_robots, num_constraints = num_constraints) )


# higher order
robots.append( DoubleIntegrator3D( np.array([-4,-2,0,0,0,0]), dt, ax, nominal_plot = nominal_plot, id = 1, color='g', alpha = alpha, mode='ego', target = np.array([0.5,0.4,0.1]).reshape(-1,1), num_robots = num_robots, num_constraints = num_constraints  ) )
